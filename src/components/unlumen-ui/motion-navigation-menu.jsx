...
function MotionNavigationMenuList({ className, highlightClassName, ...props }) {
  return (
    <Highlight
      mode="parent"
      controlledItems
      hover
      className={cn("bg-accent rounded-md pointer-events-none", highlightClassName)}
      style={{ zIndex: -1 }}
...
function MotionNavigationMenuItem({ className, value, ...props }) {
...
function MotionNavigationMenuTrigger({ className, children, onPointerEnter, onFocus, onClick, ...props }) {
...
function MotionNavigationMenuContent({ className, highlightClassName, innerClassName, children }) {
  const context = React.useContext(MotionNavigationMenuContext);
  const itemContext = React.useContext(MotionNavigationMenuItemContext);
  const value = itemContext?.value;
  const isOpen = !!value && context?.activeValue === value;
...
function MotionNavigationMenuContentInner({ highlightClassName, innerClassName, children }) {
  return (
    <Highlight
      mode="parent"
      controlledItems
      hover
      className={cn("bg-accent rounded-sm pointer-events-none", highlightClassName)}
      style={{ zIndex: -1 }}
      containerClassName="relative"
    >
...
function MotionNavigationMenuViewport({ className, contentByValue }) {
  const context = React.useContext(MotionNavigationMenuContext);
...
function MotionNavigationMenuLink({ className, ...props }) {
  return (
    <HighlightItem asChild>
      <a
...
function MotionNavigationMenuIndicator({ className, ...props }) {
  return (
    <div
      data-slot="navigation-menu-indicator"
      className={cn("pointer-events-none top-full z-1 flex h-1.5 items-end justify-center overflow-hidden", className)}
      {...props}
    >
